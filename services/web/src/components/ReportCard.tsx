...
import { BlockRenderer, UIBlock } from './blocks';
...
export const ReportCard = observer(({ report, variant = 'compact', isRelevancePick }: Props) => {
  ...
  const uiBlocks: UIBlock[] = (() => {
    try {
      return JSON.parse(report.ui_blocks_json || '[]');
    } catch {
      return [];
    }
  })();
  ...
  return (
    ...
    >
      ...
      {/* UI Blocks (A2UI) */}
      {variant === 'full' && uiBlocks.length > 0 && (
        <div className="px-4 mt-4">
          <BlockRenderer
            blocks={uiBlocks}
            onLinkClick={(url) => telemetryStore.trackClick(report.id, url)}
          />
        </div>
      )}
