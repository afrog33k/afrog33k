...
export async function visitRoutes(app) {
      ...
      `);
            for (const visit of visits) {
                try {
                    ...
                    stmt.run(id, parsed.url, parsed.title || null, host, parsed.browser, parsed.visited_at, parsed.duration_seconds || null);
                    added++;
                }
                catch (err) {
    ...
    app.post('/visits/clusters', async (request) => {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, body.started_at, body.ended_at, body.urls_json, body.stats_json, body.burst_score || 0, body.novelty_score || 0);
        return { id };
    });
    // GET /visits/clusters
