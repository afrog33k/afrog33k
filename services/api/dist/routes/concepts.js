...
export async function conceptRoutes(app) {
    const db = getDb();
    // GET /concepts
    ...
    app.post('/concepts/:id/unmerge', async (request) => {
        const { id } = request.params;
        const concept = db.prepare('SELECT merged_into_id FROM concepts WHERE id = ?').get(id);
    ...
    `).all(1 - SIMILARITY_THRESHOLD);
        ...
        for (const c of candidates) {
          ...
          VALUES (?, ?, ?, ?, ?)
        `).run(id, c.concept_id, c.neighbor_id, similarity, `High similarity (${(similarity * 100).toFixed(1)}%) between "${c.label_a}" and "${c.label_b}"`);
                created++;
            }
            catch (err) {
                // Ignore duplicates
    ...
    `).get(id);
        ...
        const conceptA = db.prepare('SELECT * FROM concepts WHERE id = ?').get(suggestion.concept_a_id);
        const conceptB = db.prepare('SELECT * FROM concepts WHERE id = ?').get(suggestion.concept_b_id);
      ...
      VALUES (?, ?, 'merged', ?, ?, ?, ?)
    `).run(randomUUID().replace(/-/g, ''), source.id, JSON.stringify(source), JSON.stringify({ ...source, active: 0, merged_into_id: target.id }), target.id, `Merge suggestion accepted: ${source.label} → ${target.label}`);
        // Perform the merge
        db.prepare(`
    ...
    app.post('/concepts/:id/kill', async (request) => {
      ...
      VALUES (?, ?, 'killed', ?, ?, ?)
    `).run(randomUUID().replace(/-/g, ''), id, JSON.stringify(concept), JSON.stringify({ ...concept, active: 0 }), body.reason || 'Manual kill');
        // Soft delete
        db.prepare(`
    ...
    app.post('/concepts/:id/revive', async (request) => {
      ...
      INSERT INTO concept_changes (id, concept_id, change_type, old_state_json, new_state_json, reason)
      VALUES (?, ?, 'revived', ?, ?, ?)
    `).run(randomUUID().replace(/-/g, ''), id, JSON.stringify(concept), JSON.stringify({ ...concept, active: 1 }), 'Manual revive');
        // Revive
        db.prepare(`
