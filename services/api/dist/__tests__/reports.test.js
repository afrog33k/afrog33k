...
describe('Reports', () => {
    ...
    describe('CRUD operations', () => {
      ...
      `).run(id, 'research_brief', 'Test Research', 1.0, 0.5, 0.2);
            const report = db.prepare('SELECT blended_score FROM reports WHERE id = ?').get(id);
      ...
      `).run(id, 'repo_signal', 'Test Repo', 0);
            ...
            const report = db.prepare('SELECT pinned FROM reports WHERE id = ?').get(id);
...
describe('Sources', () => {
    ...
    `).run('s1', 'https://example.com/page', 'Example', 'example.com', 5);
        const source = db.prepare('SELECT * FROM sources WHERE id = ?').get('s1');
...
describe('Concepts', () => {
    ...
    it('should create a concept', () => {
        ...
        const concept = db.prepare('SELECT * FROM concepts WHERE id = ?').get('c1');
    ...
    it('should track mention count', () => {
        ...
        const concept = db.prepare('SELECT mention_count FROM concepts WHERE id = ?').get('c1');
    ...
    it('should support reversible merges', () => {
        ...
        const merged = db.prepare('SELECT * FROM concepts WHERE id = ?').get('c1');
        ...
        const unmerged = db.prepare('SELECT * FROM concepts WHERE id = ?').get('c1');
...
describe('Telemetry', () => {
    ...
    it('should track UI action stats', () => {
        const action = db.prepare('SELECT * FROM ui_action_stats WHERE action = ?').get('useful');
        ...
        const updated = db.prepare('SELECT success_rate FROM ui_action_stats WHERE action = ?').get('useful');
...
describe('Job Queue', () => {
    ...
    `).get();
        ...
        const running = db.prepare(`SELECT status FROM job_queue WHERE id = ?`).get('j2');
    ...
    it('should handle job retries', () => {
        ...
        const job = db.prepare('SELECT * FROM job_queue WHERE id = ?').get('j1');
