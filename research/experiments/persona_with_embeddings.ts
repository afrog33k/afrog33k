/**
 * Experiment 2b: Persona Emergence with Semantic Embeddings
 *
 * This version uses a simple but effective cosine similarity approach
 * with TF-IDF-like weighting to simulate semantic understanding.
 *
 * RESEARCH QUESTION:
 * Does semantic similarity enable persona emergence where keywords fail?
 */

// =============================================================================
// SIMPLE SEMANTIC STORE (Simulated embeddings via TF-IDF + synonym expansion)
// =============================================================================
...
// Semantic synonym mappings for Stoic philosophy
const SEMANTIC_MAPPINGS: Record<string, string[]> = {
  // Trait synonyms
  stoic: ['calm', 'composed', 'unemotional', 'disciplined', 'patient', 'endure', 'acceptance'],
  humble: ['modest', 'unassuming', 'praise', 'honors', 'glory', 'recognition', 'fame'],
  just: ['fair', 'justice', 'right', 'moral', 'ethical', 'law', 'punishment', 'mercy'],
  rational: ['reason', 'logic', 'think', 'analyze', 'wisdom', 'rational', 'decision'],
  merciful: ['forgive', 'clemency', 'spare', 'compassion', 'enemy', 'rebel', 'mercy'],
  disciplined: ['control', 'restrain', 'moderate', 'temper', 'impulse', 'desire'],
  philosophical: ['reflect', 'contemplate', 'meaning', 'life', 'death', 'immortal', 'technology'],
  'duty-bound': ['obligation', 'responsibility', 'service', 'duty', 'power', 'authority', 'rule'],

  // Value synonyms
  virtue: ['good', 'moral', 'ethical', 'right', 'excellence', 'character'],
  duty: ['obligation', 'serve', 'responsibility', 'power', 'authority'],
  justice: ['fair', 'right', 'punishment', 'mercy', 'law', 'torture', 'dilemma'],
  wisdom: ['wise', 'understand', 'knowledge', 'learn', 'philosophical'],
  temperance: ['moderation', 'restraint', 'balance', 'excess', 'control'],

  // Behavioral patterns
  anger: ['angry', 'rage', 'fury', 'temper', 'emotion', 'upset', 'irritate'],
  praise: ['honor', 'glory', 'fame', 'recognition', 'acclaim', 'compliment'],
  wrong: ['injure', 'harm', 'offense', 'insult', 'enemy', 'adversary'],
  tempted: ['desire', 'want', 'pleasure', 'indulgence', 'weakness'],
  suffering: ['pain', 'hardship', 'struggle', 'difficulty', 'endure'],
};

class SemanticMemoryStore {
  private entries: Map<string, MemoryEntry> = new Map();
  private allTerms: Map<string, number> = new Map(); // term -> doc frequency
  ...
  private expandTokens(tokens: Set<string>): Set<string> {
    const expanded = new Set(tokens);

    for (const token of tokens) {
      // Direct synonym expansion
      if (SEMANTIC_MAPPINGS[token]) {
        for (const synonym of SEMANTIC_MAPPINGS[token]) {
          expanded.add(synonym);
        }
      }

      // Reverse mapping: if token appears in any synonym list, add the key
      for (const [key, synonyms] of Object.entries(SEMANTIC_MAPPINGS)) {
  ...
  store(id: string, content: string, type: string, metadata: Record<string, any> = {}): void {
    ...
    for (const token of tokens) {
      this.allTerms.set(token, (this.allTerms.get(token) || 0) + 1);
    }

    this.entries.set(id, { id, content, type, metadata, tokens, expandedTokens });
  }

  search(query: string, limit: number = 10): MemoryEntry[] {
    const queryTokens = this.tokenize(query);
    const expandedQuery = this.expandTokens(queryTokens);
    const results: Array<{ entry: MemoryEntry; score: number }> = [];

    for (const entry of this.entries.values()) {
      let score = 0;

      // Score based on expanded token overlap
      for (const queryToken of expandedQuery) {
        if (entry.expandedTokens.has(queryToken)) {
          // IDF-like weighting: rare terms score higher
          const df = this.allTerms.get(queryToken) || 1;
          const idf = Math.log(this.entries.size / df + 1);
          score += idf;
        }
      }

      // Boost persona-related entries (traits, values, behaviors)
      if (entry.type === 'trait' || entry.type === 'value' || entry.type === 'behavior') {
        score *= 3.0;
      } else if (entry.type === 'biography') {
        score *= 1.5;
      }
...
// =============================================================================
// PERSONA DEFINITION (Same as original)
// =============================================================================
...
// =============================================================================
// NOISE GENERATOR (Same as original)
// =============================================================================
...
async function runExperiment(): Promise<ExperimentResult> {
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('   EXPERIMENT 2b: PERSONA EMERGENCE WITH SEMANTIC EMBEDDINGS');
  console.log('═══════════════════════════════════════════════════════════════\n');
  ...
  console.log(`Semantic Mappings: ${Object.keys(SEMANTIC_MAPPINGS).length} concepts\n`);
...
function evaluateScenario(memoryStore: SemanticMemoryStore, scenario: TestScenario): ScenarioResult {
